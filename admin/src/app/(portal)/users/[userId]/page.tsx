...
import { adminUserDashboardSchema, isAdminUserId } from '@/contract/user-dashboard';
...
export default async function AdminUserDashboardPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  if (!isAdminUserId(userId)) notFound();
  const data = await adminData(`/api/admin/users/${userId}`, adminUserDashboardSchema);
  ...
  return <>
    <header className="admin-user-breadcrumb">
      <div><Link href="/users">Users</Link><span>›</span><strong>{data.user.name}</strong></div>
      <Link href="/users" className="admin-secondary-button"><ArrowLeft size={16}/> Back to Users</Link>
    </header>
