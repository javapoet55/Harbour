...
import type { AdminPlan } from '@/contract/snapshot';
...
export function AdminUsersTable({ users }: { users: UserRow[] }) {
  ...
  function exportCsv() {
    ...
    const csv = [['Name', 'Email', 'Plan', 'City', 'Country', 'Time zone', 'Sign up date (Pacific Time)', 'Last activity (Pacific Time)', 'AI actions', 'Voice minutes', 'Verified'], ...filtered.map((user) => [user.name, user.email, user.plan, user.city ?? '', user.country ?? '', user.timeZone, formatVoiceDate(user.createdAt), formatVoiceDate(user.lastActiveAt), user.aiActions, user.voiceMinutes, user.verified])].map((row) => row.map(escaped).join(',')).join('\n');
  ...
  return <>
    ...
    <div className="admin-table-scroll"><table className="admin-table admin-users-table"><thead><tr><th>User</th><th>Plan</th><th>City</th><th>Country</th><th>Sign up date · Pacific Time</th><th>Last activity · Pacific Time</th><th>AI actions</th><th>Voice minutes</th><th>Status</th></tr></thead><tbody>
      {rows.map((user) => <tr className="admin-user-row" key={user.id} tabIndex={0} aria-label={`Open dashboard for ${user.name}`} onClick={() => router.push(`/users/${user.id}`)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') router.push(`/users/${user.id}`); }}><td><div className="admin-user-cell"><span>{user.name.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase()}</span><div><strong>{user.name}</strong><small>{user.email}</small></div></div></td><td><span className={`admin-plan plan-${user.plan.toLowerCase()}`}>{user.plan === 'MAX' ? 'Max' : user.plan === 'PRO' ? 'Pro' : 'Free'}</span></td><td>{user.city ?? <span className="admin-missing-data">Not provided</span>}</td><td>{user.country ?? <span className="admin-missing-data">Not provided</span>}</td><td>{formatVoiceDate(user.createdAt)}</td><td>{formatVoiceDate(user.lastActiveAt)}</td><td>{user.aiActions.toLocaleString()}</td><td>{user.voiceMinutes.toFixed(1)}</td><td><span className={`admin-status ${user.verified ? '' : 'pending'}`}>{user.verified ? 'Verified' : 'Unverified'}</span></td></tr>)}
