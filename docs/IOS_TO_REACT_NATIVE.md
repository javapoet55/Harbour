...
`momentsStore` (Swift's Today reads the same `ImportantMomentsStore`). `app/shopping/index.tsx` is still a
title-only placeholder ("My Lists", `TODO(phase11-run-c)`) for Run C. Swift pushes both screens inside
