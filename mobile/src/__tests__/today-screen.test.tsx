...
/** Seeds the one Moments store, as the root layout's activation would. */
function setMoments(moments: ImportantMoment[]) {
  momentsStore.setState({ snapshot: { moments, emailAccount: null, emailConfigured: false, automaticEmailEnabled: false } });
}
...
describe('Today moments and shopping', () => {
  ...
  it('counts today’s moments into the commitments and the summary line', async () => {
    setMoments([moment('a', '2026-09-16'), moment('b', '2026-09-20'), moment('off', '2026-09-16', { enabled: false })]);
    await renderToday();
  ...
  it('drops the moment count on the longer ranges, as Swift passes 0', async () => {
    setMoments([moment('a', '2026-09-16')]);
    await renderToday();
    await waitFor(() => expect(screen.getByTestId('today-summary-line').props.children).toBe('1 Task · 1 Appointment · 1 Moment'));
