...
import { momentsDueToday, shoppingSubtitle, showsAttentionRow, upcomingMomentCount } from '../../../src/lib/todayQuickAccess';
import {
  buildSchedule,
  dateLabel,
  greeting,
...
import { useMoments, useShoppingLists } from '../../../src/query/useQuickAccess';
...
export default function Today() {
  ...
  // `ImportantMomentsStore` and the Quick Access `ShoppingStore` (TodayQuickAccess.swift:16-20).
  const moments = useMoments(profile !== null);
  ...
  const momentCount = range === 1 ? momentsDueToday(moments.data?.moments ?? [], queueNow).length : 0;
  ...
  return (
    <View style={styles.fill}>
      ...
      >
        ...
        <TodayQuickAccess
          momentsSubtitle={`${upcomingMomentCount(moments.data?.moments ?? [], queueNow)} upcoming`}
