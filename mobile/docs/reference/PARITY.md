...
| Ask Nexdo, suggestions (23) | default, light + dark | pending device | `ask-default.png`; `AskNexdoView.swift:182-277`, `:58-87`, `:277-310` | **None — every pixel of this screen is in `src/components/`**, which this session does not own. Reviewed in full and written up as 21 lettered requests in [`SHARED-REQUESTS.md`](SHARED-REQUESTS.md): the `AskStyle.ink`/`AskStyle.secondary` → `.label`/`.secondaryLabel` mapping (a light-mode error throughout), `.subheadline` 20→21 and `.footnote` 18→20 everywhere, the suggestion and entry card text-stack gaps, `minimumScaleFactor(0.75)`, the `.body` glyph sizes, the 0.55→0.25 disabled opacity, and the elevated palette for the sheet | `AskNexdoView.tsx`, `AskParts.tsx` — the whole group | Detents: Swift presents this at `.fraction(0.84)` with a drag indicator; Expo Router has no detent API on Android, so it is a full-height modal |
| Free form Text (23) | default | pending device | `AskNexdoView.swift:215-230`, `:312-345` — **no capture** | None — same shared file. The composer, the field's `lineLimit(4...8)` height, the example rows' `.body` font and the three `.body`-not-`.subheadline` speech controls are all in the request list | `AskNexdoView.tsx`, `AskParts.tsx` | No reference capture exists for the text page or for an answered turn, so the response cards were reviewed from source alone |
| AI consent (24) | default | pending device | `AskNexdoView.swift:348-364` — **no capture** | None — same shared file. `.buttonStyle(.borderedProminent)` is a content-sized capsule, not the full-width block built here, and "Not now" is a plain leading-aligned text button | `AskNexdoView.tsx` | `ai-consent-default.png` is mislabelled: it captures the **voice** consent alert on `AddTaskByVoiceView`, not `consentView`. Screen 24 still needs a capture. `[.medium, .large]` detents have no Android equivalent |
