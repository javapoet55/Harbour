...
final class RealtimeTaskAudio: @unchecked Sendable {
    ...
    private var readyBell: AVAudioPlayer?
    ...
    /// Called only after capture has successfully started or resumed.
    func playReadyCue() {
        queue.async { [self] in
            guard engine?.isRunning == true,
                  let url = Bundle.main.url(forResource: "ListeningReady", withExtension: "wav") else { return }
            readyBell = try? AVAudioPlayer(contentsOf: url)
            readyBell?.volume = 0.65
            readyBell?.play()
        }
    }
    func pause() { queue.async { [self] in readyBell?.stop(); engine?.pause() } }
    func stop() { queue.async { [self] in teardown() } }
    private func teardown() {
        readyBell?.stop(); readyBell = nil
