...
export function ScheduleIntelligenceCard({ onChanged }: { onChanged: () => void }) {
  ...
  return <section className="schedule-intelligence" aria-label="Schedule Intelligence">
    ...
    {expanded && <div className="schedule-intelligence-details">
      {data?.conflicts.length ? data.conflicts.map((conflict) => <article key={conflict.id} className={`schedule-conflict severity-${conflict.severity.toLowerCase()}`}><p><b>{conflict.title}</b><span>{conflict.type.toLowerCase()} · {Math.round(conflict.confidence * 100)}% confidence</span></p><div>{conflict.explanation}</div><small>Recommended: {conflict.recommendedAction}</small></article>) : <p className="schedule-clear">No overlap, travel-buffer, workload, or priority conflicts were found.</p>}
      {proposal && <div className="schedule-proposal"><h3>Proposed changes</h3><p>Nothing changes until you approve. Connected calendar events are recommendations only.</p>{proposal.moves.length ? proposal.moves.map((move) => <div key={move.taskId}><strong>{move.title}</strong><span>{move.fromLabel} → {move.toLabel}</span></div>) : <p>No Harbor task moves are needed.</p>}{proposal.risks.map((risk) => <p key={risk.taskId}>{risk.title}: {risk.reason === 'no_capacity' ? 'still needs a time block' : 'may miss its deadline'}</p>)}{proposal.actionId && <button type="button" className="harbor-btn harbor-btn-brand" disabled={busy} onClick={() => void apply()}>{busy ? 'Applying…' : `Approve ${proposal.moves.length} task change${proposal.moves.length === 1 ? '' : 's'}`}</button>}</div>}
    </div>}
