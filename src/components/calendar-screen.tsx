...
export function CalendarScreen() {
  ...
  useEffect(() => {
    ...
    scheduleFetch(`/api/agenda?days=${count}${from ? `&from=${from}` : ''}`, { signal: controller.signal }).then(async (res) => { const payload = await res.json(); if (!res.ok) throw new Error(payload.error || 'Could not load your calendar.'); return payload as AgendaPayload; }).then((payload) => {
  ...
  async function move(task: AgendaTask, day: string) {
    ...
    try {
      const res = await scheduleFetch(`/api/tasks/${task.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ date: day, time: '09:00', durationMin: task.durationMin }) });
