...
// Sort the limited Google selection by absolute dates; undated reviews stay last.
function reviewTime(value:string|undefined){
...
function googleCandidate(r:z.infer<typeof placeSchema>):Candidate{
 ...
 feedback:(r.reviews??[]).filter(v=>safeUrl(v.googleMapsUri)).sort((a,b)=>{const left=reviewTime(a.publishTime),right=reviewTime(b.publishTime);return left===right?0:right-left;}).map(v=>({author:v.authorAttribution.displayName,authorUrl:safeUrl(v.authorAttribution.uri),photoUrl:safeUrl(v.authorAttribution.photoUri),url:safeUrl(v.googleMapsUri)!,rating:v.rating??null,text:v.text?.text??'',published:v.relativePublishTimeDescription??''})),
