...
export const itemInput=z.object({id:z.string().uuid(),name:z.string().trim().min(1).max(120),category:z.enum(categories).default('Other'),quantity:z.string().trim().max(40).default('1'),size:z.string().trim().max(80).default(''),notes:z.string().trim().max(300).default(''),checked:z.boolean().default(false)});
