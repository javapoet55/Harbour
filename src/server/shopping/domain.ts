...
export const itemInput=z.object({id:z.string().uuid(),name:z.string().trim().min(1).max(120),category:z.enum(categories).default('Other'),quantity:z.string().trim().max(40).default('1'),size:z.string().trim().max(80).default(''),notes:z.string().trim().max(300).default(''),imageData:z.string().max(90000).regex(/^\/9j\/[A-Za-z0-9+/]*={0,2}$/).nullable().optional(),checked:z.boolean().default(false),barcode:z.string().regex(/^\d{8,14}$/).nullish().transform(v=>v??undefined),brand:z.string().trim().max(120).nullish().transform(v=>v??undefined),favorite:z.boolean().nullish().transform(v=>v??undefined),favoriteAlternatives:z.array(z.string().max(400)).max(100).nullish().transform(v=>v??undefined)});
