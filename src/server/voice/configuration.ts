...
export const voiceTools = [
  ...
  tool('create_calendar_event', 'Create a NexDo calendar event, not an external booking or invitation. Clarify title, exact start and end first.', { title: task.title, notes: task.notes, startAt: time, endAt: time, location: str('Optional location') }, ['title', 'startAt', 'endAt']),
  tool('create_reminder', 'Create a task reminder at the explicitly requested time. Ask for exact date/time if ambiguous.', { title: task.title, notes: task.notes, categoryName: task.categoryName, scheduledAt: time }, ['title', 'scheduledAt']),
