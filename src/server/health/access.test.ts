...
it('permits admin read access but restricts elevated mutations',async()=>{vi.mocked(readAdminSession).mockResolvedValue({id:'admin'} as NonNullable<Awaited<ReturnType<typeof readAdminSession>>>);vi.stubEnv('NEXDO_HEALTH_OPERATOR_IDS','');await expect(healthAccess()).resolves.toMatchObject({id:'admin'});await expect(healthAccess(true)).rejects.toThrow('FORBIDDEN');vi.stubEnv('NEXDO_HEALTH_OPERATOR_IDS','admin');await expect(healthAccess(true)).resolves.toMatchObject({id:'admin'});});
