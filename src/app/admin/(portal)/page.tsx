...
export default async function AdminOverviewPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  return <>
    ...
    <div className="admin-grid">
      ...
      <Panel title="Recent signups" className="admin-span-6" action={<Link href={`/admin/users?from=${range.from}&to=${range.to}`}>View all</Link>}><table className="admin-list"><tbody>{data.users.filter((user) => new Date(user.createdAt) >= range.fromDate && new Date(user.createdAt) <= new Date(`${range.to}T23:59:59.999Z`)).slice(0,6).map((user)=><tr key={user.id}><td>{user.email}</td><td>{user.plan}</td><td>{new Date(user.createdAt).toLocaleDateString()}</td></tr>)}</tbody></table></Panel>
