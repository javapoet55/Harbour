...
it('accepts an operator mutation from the admin frontend, which sends no Origin',async()=>{
 const body=JSON.stringify({type:'incident',id:'incident-1',action:'ACKNOWLEDGED'});
 vi.mocked(healthAccess).mockResolvedValue({id:'operator'} as Awaited<ReturnType<typeof healthAccess>>);
