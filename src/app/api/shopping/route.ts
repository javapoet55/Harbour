...
export async function POST(req:Request){try{const user=await requireUser();const body=await req.text();if(body.length>4000000)return Response.json({error:'List is too large.'},{status:413});return Response.json(await shoppingAction(user.id,JSON.parse(body)),{headers})}catch(e){return failure(e)}}
